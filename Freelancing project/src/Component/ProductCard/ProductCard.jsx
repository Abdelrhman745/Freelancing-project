
import { Link } from 'react-router-dom';
import FavoriteIcon from '@mui/icons-material/Favorite';
import IconButton from '@mui/material/IconButton';
import Badge from '@mui/material/Badge';
import Style from './ProductCard.module.css'


export default function ProductCard({product}) {
  
let {id,title,price,image,category} = product;
 

  return (
    <>
     
      <div className=" mb-4 col-12" style={{ height: 450}} dir='rtl'>
        <div className={`card p-4 border-0 rounded rounded-4`}  >
          <img src={image} className="card-img-top  ms-xl-3  p-3" alt="..." style={{height:300}}/>
          <div className="card-body">
            <h4 className="card-title fs-4 fw-bold text-center">{title.split(" ").splice(0,2).join(" ")}</h4>
            <div className='d-flex justify-content-between mt-3'>
              <p className='fw-bold'>{price} <span className='text-success'>جنيها</span></p>
              <p className="fs-6">
                <span><i class="fa-solid fa-star-half text-warning"></i></span>
                 {[...Array(4)].map((_, i) => (
                   <i key={i} className="fa-solid fa-star text-warning"></i>
                 ))} 
              </p>
            </div >
            <Link to={`/product/${id}/${category}`} className="btn btn-sm text-white w-100 p-0 " style={{backgroundColor:"#212529"}}>
            <h6 className='mt-2'>تفاصيل المنتج</h6>
          </Link>
          </div>
        </div>
       </div>


        

    </>
  )
}
